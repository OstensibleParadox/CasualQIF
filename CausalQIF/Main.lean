...
theorem condMutualInfo_eq_zero_of_factorizes_of_dSeparated
...
theorem stateLeakage_le_of_factorizes_of_dSeparated_of_cutMutualInfo_le
    ...
    (C : ℝ)
    (h_factor : FactorizesOverDAG G isMarkovChainNodeCI P3)
    (h_dsep : dSeparates G ({0} : Finset ℕ) ({2} : Finset ℕ) ({1} : Finset ℕ))
...
theorem linearChain_stateLeakage_le_one_of_dSeparated
    (G : DAG)
    (P : FinitePMF (State × VisibleTrace × MissingTrace))
    (P3 : FinitePMF (State × VisibleTrace × MissingTrace))
    (h_factor : FactorizesOverDAG G isMarkovChainNodeCI P3)
